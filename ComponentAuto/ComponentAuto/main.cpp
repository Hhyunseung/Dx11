...
int main(int Argc, char** Argv)
{
    ...
    {
        const bool bOk = InsertLinesAfterMarkerLine(GameObjectText, "// 특정 컴포넌트를 다운캐스팅해서 바로 리턴", GameObjectMacroLinesToInsert);
        if (!bOk)
        {
            std::cout << "ERROR: Marker not found in GameObject.h: \"// 특정 컴포넌트를 다운캐스팅해서 바로 리턴\"\n";
            std::string Dummy;
            std::getline(std::cin, Dummy);
            return 1;
        }
