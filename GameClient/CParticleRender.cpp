	...
	, m_ModuleChanged(false)
{
	...
	Ptr<AComputeShader> pCS = AssetMgr::GetInst()->FindAsset<AComputeShader>(L"ParticleTickCS");
...
{
	if (nullptr == GetMesh())
		SetMesh(AssetMgr::GetInst()->FindAsset<AMesh>(L"PointMesh"));

	if (nullptr != GetMaterial())
		return;

	Ptr<AMaterial> pMtrl = AssetMgr::GetInst()->FindAsset<AMaterial>(L"ParticleMtrl");
	if (nullptr != pMtrl)
	{
		SetMaterial(pMtrl);
		return;
	...
	pShader->CrateGeometryShader(L"Shader\\particle.fx", "GS_Particle");
