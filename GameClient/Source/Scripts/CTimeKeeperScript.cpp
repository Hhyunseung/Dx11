...
void CTimeKeeperScript::SpawnTKParticle(int _LayerIdx)
{
	DestroyTKParticle();
	...
	m_TKParticleObject->Transform()->SetRelativePos(Vec3(300.f, 0.f, -100.f));


	// 파티클 입자에 입힐 텍스쳐 설정
	Ptr<CParticleRender> pRender = m_TKParticleObject->ParticleRender();
	Ptr<ATexture> m_TKParticleObjectTex = AssetMgr::GetInst()->Load<ATexture>(L"TK_Particle", L"Texture\\particle\\TimeKeeper_Particle.png");
	...
	pRender->SetBurstRepeatCount(100);
...
void CTimeKeeperScript::DestroyTKParticle()
{
